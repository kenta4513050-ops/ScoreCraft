...
function saveCourseFromForm(){const name=document.getElementById("courseName").value.trim();const pars=getPars();if(!name){message("ゴルフ場名を入力してください。",true);return;}if(pars.some(v=>![3,4,5,6].includes(v))){message("18ホールすべてのPARを選択してください。",true);return;}try{const id=document.getElementById("editingCourseId").value||createCourseId();upsertCustomCourse({id,name,prefecture:document.getElementById("prefecture").value.trim(),courseName:document.getElementById("courseNameDetail").value.trim(),isCustom:true,holes:pars.map((par,i)=>({hole:i+1,par}))});const saved=getCourseById(id);if(!saved){throw new Error("保存内容を確認できませんでした。");}renderCourseList();message("ゴルフ場を保存しました。設定画面に戻ります。");resetForm(false);window.setTimeout(()=>{location.href="settings.html";},500);}catch(error){console.error("ゴルフ場を保存できませんでした。",error);message("ゴルフ場を保存できませんでした。もう一度お試しください。",true);}}
