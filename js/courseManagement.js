...
function initCourseManager(){
    ...
    createParInputs();
    renderCourseList();document.getElementById("saveCourseButton").addEventListener("click",saveCourseFromForm);document.getElementById("cancelEditButton").addEventListener("click",resetForm);document.getElementById("clearParButton").addEventListener("click",()=>setPars(Array(18).fill("")));document.getElementById("applyParSequenceButton").addEventListener("click",applyParSequence);const seq=document.getElementById("parSequenceInput");seq.addEventListener("input",()=>{seq.value=seq.value.replace(/[^3-6]/g,"").slice(0,18);if(seq.value.length===18)applyParSequence(false);});}
...
function renderCourseList(){
    ...
    box.innerHTML=courses.map(c=>`<article class="course-manager-item"><div><strong>${escapeHtml(c.name)}</strong><small>${escapeHtml([c.frontCourseName,c.backCourseName].filter(Boolean).join(" / ") || c.courseName || "")}</small><span>PAR ${c.holes.reduce((sum,h)=>sum+Number(h.par||0),0)}</span></div><div class="course-manager-actions"><button type="button" class="mini-button" onclick="editCourse('${c.id}')">編集</button><button type="button" class="mini-button danger-mini" onclick="removeCourse('${c.id}')">削除</button></div></article>`).join("");
}
...
function saveCourseFromForm(){const name=document.getElementById("courseName").value.trim();const pars=getPars();if(!name){message("ゴルフ場名を入力してください。",true);return;}if(pars.some(v=>![3,4,5,6].includes(v))){message("18ホールすべてのPARを選択してください。",true);return;}try{const id=document.getElementById("editingCourseId").value||createCourseId();upsertCustomCourse({id,name,frontCourseName:document.getElementById("frontCourseName").value.trim(),backCourseName:document.getElementById("backCourseName").value.trim(),courseName:[document.getElementById("frontCourseName").value.trim(),document.getElementById("backCourseName").value.trim()].filter(Boolean).join(" / "),isCustom:true,holes:pars.map((par,i)=>({hole:i+1,par}))});const saved=getCourseById(id);if(!saved){throw new Error("保存内容を確認できませんでした。");}renderCourseList();message("ゴルフ場を保存しました。設定画面に戻ります。");resetForm(false);window.setTimeout(()=>{location.href="settings.html";},500);}catch(error){console.error("ゴルフ場を保存できませんでした。",error);message("ゴルフ場を保存できませんでした。もう一度お試しください。",true);}}
function editCourse(id){const c=getCourseById(id);if(!c||!isCustomCourse(id))return;document.getElementById("editingCourseId").value=c.id;document.getElementById("courseName").value=c.name;const legacyNames=String(c.courseName||"").split("/").map(v=>v.trim());document.getElementById("frontCourseName").value=c.frontCourseName||legacyNames[0]||"";document.getElementById("backCourseName").value=c.backCourseName||legacyNames[1]||"";setPars(c.holes.map(h=>h.par));document.getElementById("formTitle").textContent="ゴルフ場を編集";document.getElementById("cancelEditButton").hidden=false;document.getElementById("courseFormCard").scrollIntoView({behavior:"smooth"});}
